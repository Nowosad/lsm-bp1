...
<!-- For example, ... .  -->
In this post, I show how to prepare input data and how to calculate selected landscape metrics based on different buffer size and shape.
...
In the same time, the **landscapemetrics** package has many more to offer, including [a lot of implemented landscape metrics](https://r-spatialecology.github.io/landscapemetrics/reference/index.html), a number of utility functions (e.g. `extract_lsm()` or `get_patches()`), and several C++ functions design for the development of new metrics. 
