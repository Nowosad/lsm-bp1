...
As a default, it calculates all of the available metrics in a square buffer:
...
... or just a single metric, for example SHDI (Shannon's diversity index) on a landscape level (`"lsm_l_shdi"`):
...
<!-- more options to specify options -->
As a default, it uses a square buffer, but it could be changed to a circle one with the `shape` argument set to `"circle"`.

# Calculations of landscape metrics for many buffers

Finally, it is also possible to calculate landscape metrics on different buffer sizes.
You need to select what sizes you are interested in, and next use the `sample_lsm()` function inside a `map_dfr()` function.
...
The result is a data frame, where the first column describes the buffer size, and the rest is a strandard **landscapemetrics** output.
