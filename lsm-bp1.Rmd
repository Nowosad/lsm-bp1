...
A landscape level metric gives just one value describing cetrain property of a landscape mosaic, such as its diversity. 
...
<!-- For example, ... .  -->
In this post, I show how to prepare input data and how to calculate selected landscape metrics based on a different buffer size and shape.
All calculations were performed in R, using **gdalUtils**, **sf**, and **raster** packages for spatial data handling, **purrr** to ease some calculations, and most importantly the **landscapemetrics** packge was used to calculate landscape metrics.
...
As a default, it calculates all of the available metrics in a square buffer, where a buffer size is the side-length in map units (e.g. meters).
...
The structure of the output is stable and contains following columns:

1. `layer` - an id of the input raster (landscape). It is possible to calculate landscape metrics on a several rasters at the same time.
...
7. `plot_id` - an id of the input vector (point).
8. `percentage_inside` - a size of the actual sampled landscape in %. It is not always 100% due to two reasons: (i) clipping raster cells using a buffer not directly intersecting a raster cell center lead to inaccuracies and (ii) sample plots can exceed the landscape boundary.
...
The `sample_lsm()` function also allows for selecting metrics using a level name (`"landscape"`, `"class"`, or `"patch"`), a metric abbriviation, full name of a metric, or a metric type. 
...
As default, `sample_lsm()` uses a square buffer, but it could be changed to a circle one with the `shape` argument set to `"circle"`.
